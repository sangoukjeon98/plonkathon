...
class Prover:
    ...
    def round_2(self) -> Message2:
        ...
        for i in range(group_order):
            Z_values.append(
                ...
                (self.C.values[i] + self.beta *
                 self.pk.S3.values[i] + self.gamma)
            )
    ...
    def round_3(self) -> Message3:
        ...
        self.QL_big, self.QR_big, self.QM_big, self.QO_big, self.QC_big = \
            (self.fft_expand(x)
             for x in (self.pk.QL, self.pk.QR, self.pk.QM, self.pk.QO, self.pk.QC))
        ...
        Z_shifted_big = Polynomial(
            self.Z_big.values[4:] + self.Z_big.values[:4], Basis.LAGRANGE)
        ...
        QUOT_big = Polynomial([((
            ...
            (self.C_big.values[i] + self.beta * 3 *
             self.fft_cofactor * self.quarter_roots[i] + self.gamma)
            ...
            (self.C_big.values[i] + self.beta *
             self.S3_big.values[i] + self.gamma)
        ...
        self.T1 = Polynomial(
            all_coeffs.values[:group_order], Basis.MONOMIAL).fft()
        self.T2 = Polynomial(
            (all_coeffs.values[group_order: group_order * 2]), Basis.MONOMIAL).fft()
        self.T3 = Polynomial(
            (all_coeffs.values[group_order * 2: group_order * 3]), Basis.MONOMIAL).fft()

        # Sanity check that we've computed T1, T2, T3 correctly
        assert (
            self.T1.barycentric_eval(self.fft_cofactor)
            + self.T2.barycentric_eval(self.fft_cofactor) *
            self.fft_cofactor**group_order
            + self.T3.barycentric_eval(self.fft_cofactor) *
    ...
    def round_4(self) -> Message4:
        ...
        self.z_shifted_eval = self.Z.barycentric_eval(
            self.zeta * roots_of_unity[1])
    ...
    def round_5(self) -> Message5:
        ...
        self.L1_eval = Polynomial([Scalar(
            1)] + [Scalar(0)] * (group_order - 1), Basis.LAGRANGE).barycentric_eval(self.zeta)
        ...
        W_zw_big = Polynomial([
            ...
            (self.fft_cofactor *
             self.quarter_roots[i] - self.zeta * roots_of_unity[1])

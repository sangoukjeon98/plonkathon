...
if TYPE_CHECKING:
    from py_ecc.ecc_typing import (  # noqa: F401
...
class FQ(object):
    ...
    def __div__(self: T_FQ, other: IntOrFQ) -> T_FQ:
        ...
        return type(self)(
            self.n *
            prime_field_inv(on, self.field_modulus) % self.field_modulus
        )
    ...
    def __rdiv__(self: T_FQ, other: IntOrFQ) -> T_FQ:
        ...
        return type(self)(
            prime_field_inv(self.n, self.field_modulus) *
            on % self.field_modulus
        )
...
class FQP(object):
    ...
    def __mul__(self: T_FQP, other: Union[int, T_FQ, T_FQP]) -> T_FQP:
        if isinstance(other, int_types_or_FQ):
            ...
            b = [self.FQP_corresponding_FQ_class(
                0) for i in range(self.degree * 2 - 1)]
            ...
            while len(b) > self.degree:
                ...
                for i in range(self.degree):
                    b[exp + i] -= top * \
                        self.FQP_corresponding_FQ_class(self.modulus_coeffs[i])
            return type(self)(b)
        else:
            raise TypeError(
    ...
    def inv(self: T_FQP) -> T_FQP:
        ...
        while deg(low):
            ...
            if len(lm) != self.degree + 1:
                raise Exception(
                    "Length of lm is not {}".format(self.degree + 1))
            elif len(hm) != self.degree + 1:
                raise Exception(
                    "Length of hm is not {}".format(self.degree + 1))
            elif len(nm) != self.degree + 1:
                raise Exception(
                    "Length of nm is not {}".format(self.degree + 1))
            elif len(low) != self.degree + 1:
                raise Exception(
                    "Length of low is not {}".format(self.degree + 1))
            elif len(high) != self.degree + 1:
                raise Exception(
                    "Length of high is not {}".format(self.degree + 1))
            elif len(new) != self.degree + 1:
                raise Exception(
                    "Length of new is not {}".format(self.degree + 1))

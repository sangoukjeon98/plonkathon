...
def mod_int(x: IntOrFQ, n: int) -> int:
    if isinstance(x, int):
        ...
        raise TypeError(
            "Only int and T_FQ types are accepted: got %s" % type(x))
...
class FQ(object):
    ...
    def __mod__(self: T_FQ, other: IntOrFQ) -> T_FQ:
        raise NotImplementedError(
            "Modulo Operation not yet supported by fields")
...
class FQP(object):
    ...
    def __div__(self: T_FQP, other: Union[int, T_FQP]) -> T_FQP:
        if isinstance(other, int):
            return type(self)([
                int(c) * prime_field_inv(other,
                                         self.field_modulus) % self.field_modulus
                for c
                in self.coeffs
            ])
    ...
    def inv(self: T_FQP) -> T_FQP:
        ...
        while deg(low):
            r = cast(List[IntOrFQ], list(
                self.optimized_poly_rounded_div(high, low)))
...
class FQ2(FQP):
    ...
    def __init__(self, coeffs: Sequence[IntOrFQ]) -> None:
        ...
        self.mc_tuples = [(i, c)
                          for i, c in enumerate(self.FQ2_MODULUS_COEFFS) if c]
...
class FQ12(FQP):
    ...
    def __init__(self, coeffs: Sequence[IntOrFQ]) -> None:
        ...
        self.mc_tuples = [(i, c)
                          for i, c in enumerate(self.FQ12_MODULUS_COEFFS) if c]
